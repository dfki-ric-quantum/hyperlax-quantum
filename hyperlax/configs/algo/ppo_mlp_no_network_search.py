...
def get_base_hyperparam_distributions() -> dict[str, Any]:
    return {
        ...
        # vec integers
        "algorithm.hyperparam.rollout_length": Categorical(values=[2, 4, 8, 16]),
        "algorithm.hyperparam.epochs": Categorical(values=list(range(1, 10))),
        # "epochs": Categorical(values=[1, 5, 10]),
        "algorithm.hyperparam.num_minibatches": Categorical(values=[16, 32, 64, 128, 256]),
        "algorithm.hyperparam.total_num_envs": Categorical(values=[1024, 2048, 4096, 8192]),
        # vec booleans
