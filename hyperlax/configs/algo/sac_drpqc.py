...
def get_base_config() -> BaseExperimentConfig:
    """Returns a GENERIC base config for SAC with a DRPQC network."""
    algorithm_component = SACConfig(
        network=SACDRPQCActorCriticConfig(), hyperparam=SACHyperparams()
    )
    ...
    config = BaseExperimentConfig(
        ...
        logger=LoggerConfig(base_exp_path="results_base/sac_drpqc_base"),
        config_name="sac_drpqc_base",
...
def get_base_hyperparam_distributions() -> dict[str, Any]:
    return {
        ...
        # DRPQC specific tunables (non-vectorized in this config)
...
def get_base_hyperparam_distributions_test_quick() -> dict[str, Any]:
    return {
        ...
        "algorithm.hyperparam.max_grad_norm": UniformContinuous(domain=(0.1, 1.0)),
        "algorithm.hyperparam.target_entropy_scale": UniformContinuous(domain=(0.8, 1.0)),
        ...
        # SACNonVecHyperparams (only autotune)
        "algorithm.hyperparam.autotune": Categorical(values=[True]),
        # DRPQC specific tunables (quick test)
        "algorithm.network.actor_network.pre_torso.n_layers": Categorical(values=[2]),
