...
from hyperlax.configs.network.actorcritic_vec_mlp import MLPActorCriticConfig
...
def get_base_hyperparam_distributions() -> dict[str, Any]:
    return {
        ...
        # vec integers
        "algorithm.hyperparam.rollout_length": Categorical(values=[16, 32, 64]),
        "algorithm.hyperparam.epochs": Categorical(values=[1, 2, 4]),
        "algorithm.hyperparam.num_minibatches": Categorical(values=[64, 128, 256]),
        "algorithm.hyperparam.total_num_envs": Categorical(values=[32, 64, 128]),
        # vec booleans
        ...
        # non-vec hyperparams
        "algorithm.network.actor_network.pre_torso.num_layers": UniformDiscrete(domain=(1, 3)),
        "algorithm.network.actor_network.pre_torso.width": Categorical(values=[64, 128, 256]),
        ...
        "algorithm.network.critic_network.pre_torso.num_layers": UniformDiscrete(domain=(1, 3)),
        "algorithm.network.critic_network.pre_torso.width": Categorical(values=[64, 128, 256]),
        ...
        # # test w/o
        # "mlp.actor_network.pre_torso.layer_sizes": Categorical(values=[[128,128]]),
        # "mlp.actor_network.pre_torso.activation": Categorical(values=["relu"]),
        # "mlp.actor_network.pre_torso.use_layer_norm": Categorical(values=[False]),
        # "mlp.critic_network.pre_torso.layer_sizes": Categorical(values=[[128,128]]),
        # "mlp.critic_network.pre_torso.activation": Categorical(values=["relu",]),
        # "mlp.critic_network.pre_torso.use_layer_norm": Categorical(values=[False]),
    }
...
def get_base_hyperparam_distributions_test_quick() -> dict[str, Any]:
    return {
        ...
        "algorithm.hyperparam.standardize_advantages": Categorical(values=[True, False]),
        "algorithm.hyperparam.decay_learning_rates": Categorical(values=[True, False]),
        "algorithm.hyperparam.normalize_observations": Categorical(values=[True, False]),
        # non-vec hyperparams
        "algorithm.network.actor_network.pre_torso.num_layers": Categorical(values=[2]),
        "algorithm.network.actor_network.pre_torso.width": Categorical(values=[16]),
        ...
        "algorithm.network.critic_network.pre_torso.num_layers": Categorical(values=[2]),
        "algorithm.network.critic_network.pre_torso.width": Categorical(values=[16]),
