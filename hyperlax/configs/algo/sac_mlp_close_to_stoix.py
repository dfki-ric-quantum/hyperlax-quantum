...
def get_base_hyperparam_distributions() -> dict[str, Any]:
    return {
        ...
        "algorithm.hyperparam.actor_lr": LogUniform(domain=(3e-4, 4e-4)),
        "algorithm.hyperparam.q_lr": LogUniform(domain=(3e-4, 4e-4)),
        "algorithm.hyperparam.alpha_lr": LogUniform(domain=(3e-4, 4e-4)),
        "algorithm.hyperparam.gamma": UniformContinuous(domain=(0.99, 0.999)),
        "algorithm.hyperparam.tau": UniformContinuous(domain=(0.005, 0.006)),
        "algorithm.hyperparam.max_grad_norm": UniformContinuous(domain=(0.5, 0.6)),
        "algorithm.hyperparam.target_entropy_scale": UniformContinuous(domain=(1.0, 1.1)),
        "algorithm.hyperparam.init_alpha": LogUniform(domain=(0.1, 0.2)),
        ...
        "algorithm.hyperparam.total_buffer_size": Categorical(values=[25000]),
        "algorithm.hyperparam.total_batch_size": Categorical(values=[256]),
        ...
        "algorithm.network.actor_network.pre_torso.activation": Categorical(values=["silu"]),
        ...
        "algorithm.network.critic_network.pre_torso.activation": Categorical(values=["silu"]),
...
def get_base_hyperparam_distributions_test_quick() -> dict[str, Any]:
    return {
        ...
        "algorithm.hyperparam.autotune": Categorical(values=[True]),
        # MLPTorso parameters (non-vectorized in this config)
        "algorithm.network.actor_network.pre_torso.layer_sizes": Categorical(values=[[8, 8]]),
        "algorithm.network.actor_network.pre_torso.activation": Categorical(values=["relu"]),
        "algorithm.network.actor_network.pre_torso.use_layer_norm": Categorical(values=[False]),
        "algorithm.network.critic_network.pre_torso.layer_sizes": Categorical(values=[[8, 8]]),
