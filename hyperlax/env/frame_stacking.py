...
class FrameStackingWrapper(Wrapper):
    ...
    def __init__(self, env: Environment, num_frames: int = 4, flatten: bool = True):
        ...
        self._stacker = FrameStacker(
            ...
            flatten=flatten,
            frame_shape=original_spec.agent_view.shape,
        )
        self._num_frames = num_frames
        self._flatten = flatten
    ...
    def update_spec(self, spec: specs.Spec) -> specs.Spec:
        ...
        if type(spec) is specs.Array:
            ...
            if spec.minimum.shape != ():
                new_minimum = jnp.repeat(spec.minimum, self._num_frames, axis=-1).reshape(
                    new_shape
                )
                new_maximum = jnp.repeat(spec.maximum, self._num_frames, axis=-1).reshape(
                    new_shape
                )
            else:
    ...
    def step(
        ...
        return (
            FrameStackEnvState(env_state=env_state, stack_state=new_stack_state),
            timestep,
        )
