...
from hyperlax.network.distributions import (
...
class DiscreteQArchitectureHead(nn.Module):
    action_dim: int
    epsilon: float = 0.1
    kernel_init: Initializer = orthogonal(1.0)
...
class DistributionalDiscreteQArchitecture(nn.Module):
    action_dim: int
    epsilon: float
    num_atoms: int
    v_min: float
...
class DistributionalContinuousQArchitecture(nn.Module):
    num_atoms: int
    v_min: float
    v_max: float
    kernel_init: Initializer = lecun_normal()
...
class QuantileDiscreteQArchitecture(nn.Module):
    action_dim: int
    epsilon: float
    num_quantiles: int
    kernel_init: Initializer = lecun_normal()
